...
class GCodeEntry(wx.Window): 
	def __init__(self, parent):
		...
		self.bSend.SetToolTip("Send a G Code command to the printer")
		...
		self.bClear.SetToolTip("Clear the G Code entry field")
		self.Bind(wx.EVT_BUTTON, self.onClear, self.bClear)

		sizerGCode.AddSpacer(10)
	...
	def evtGCodeSend(self, evt):
		cmd = self.tGCode.GetValue().strip()
		if cmd != "":
			self.reprap.sendNow(cmd, True)
